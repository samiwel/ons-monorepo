...
describe("Questionnaire", () => {
  ...
  it("should add user defined metadata", () => {
    const questionnaireJson = createQuestionnaireJSON({
      metadata: [
        { key: "example_date", type: "Date" },
        { key: "example_text", type: "Text" },
        { key: "example_region", type: "Region" },
        { key: "example_language", type: "Language" }
      ]
    });
    ...
    expect(new Questionnaire(questionnaireJson)).toHaveProperty("metadata", [
      ...
      { name: "example_date", validator: "date" },
      { name: "example_text", validator: "string" },
      { name: "example_region", validator: "string" },
      { name: "example_language", validator: "string" }
    ]);
  });

  it("should not overwrite the default metadata", () => {
    const questionnaireJson = createQuestionnaireJSON({
      metadata: [{ key: "ru_name", type: "Date" }]
    });
